...
func b_delete_file(args ...any) any {
	path := args[0].(string)
...
func b_delete_all(args ...any) any {
	path := args[0].(string)
...
func b_get_env(args ...any) any {
	return os.Getenv(args[0].(string))
}

func b_set_env(args ...any) any {
	os.Setenv(args[0].(string), args[1].(string))
	return nil
}

func b_env_vars(_ ...any) any {
	result := map[string]any{}
...
func b_now_ms(_ ...any) any {
	return time.Now().UnixMilli()
}

func b_sleep_ms(args ...any) any {
	ms := args[0].(int64)
	time.Sleep(time.Duration(ms) * time.Millisecond)
	return nil
}

func b_format_time(args ...any) any {
	unixMs := args[0].(int64)
	format := args[1].(string)
	t := time.UnixMilli(unixMs)
	// Map common format tokens to Go layout
	layout := format
...
func b_regex_match(args ...any) any {
...
func b_regex_find(args ...any) any {
	pattern := args[0].(string)
	s := args[1].(string)
	...
	if match == nil {
		return &Result{Ok: true, Value: []any{}}
	}
...
func b_regex_replace(args ...any) any {
	pattern := args[0].(string)
	s := args[1].(string)
	repl := args[2].(string)
	re, err := regexp.Compile(pattern)
	...
	return &Result{Ok: true, Value: re.ReplaceAllString(s, repl)}
}

func b_regex_find_all(args ...any) any {
	pattern := args[0].(string)
	s := args[1].(string)
	re, err := regexp.Compile(pattern)
	if err != nil {
...
func b_exec(args ...any) any {
	command := args[0].(string)
	cmd := os_exec.Command("sh", "-c", command)
...
func fn_is_builtin(args ...any) any {
	...
	if valuesEqual(v_name, "mkdir") {
		return true
	}
	if valuesEqual(v_name, "readdir") {
		return true
	}
	if valuesEqual(v_name, "file_exists") {
		return true
	}
	if valuesEqual(v_name, "delete_file") {
		return true
	}
	if valuesEqual(v_name, "delete_all") {
		return true
	}
	if valuesEqual(v_name, "stat") {
		return true
	}
	if valuesEqual(v_name, "get_env") {
		return true
	}
	if valuesEqual(v_name, "set_env") {
		return true
	}
	if valuesEqual(v_name, "env_vars") {
		return true
	}
	if valuesEqual(v_name, "now_ms") {
		return true
	}
	if valuesEqual(v_name, "sleep_ms") {
		return true
	}
	if valuesEqual(v_name, "format_time") {
		return true
	}
	if valuesEqual(v_name, "regex_match") {
		return true
	}
	if valuesEqual(v_name, "regex_find") {
		return true
	}
	if valuesEqual(v_name, "regex_replace") {
		return true
	}
	if valuesEqual(v_name, "regex_find_all") {
		return true
	}
	if valuesEqual(v_name, "exec") {
		return true
	}
	if valuesEqual(v_name, "exit") {
		return true
	}
	return false
}
